...
var wavefunction = new QuantumWavefunction(k: 1.0, mass: 9.11e-31); // Example parameters for an electron

Console.WriteLine($"Probability density: {wavefunction.ProbabilityDensity(1.2, 0.5)}"); // Compute probability density at x=1.2 m, t=0.5 s 

public class QuantumWavefunction(double k, double mass, double hbar = 1.0545718e-34)
{
    // hbar Reduced Planck's constant (J·s)
    // Particle mass (kg)
    private readonly double _k = k;    // Wavenumber (1/m)
    private readonly double _omega = (hbar * k * k) / (2 * mass); // Angular frequency (rad/s)
